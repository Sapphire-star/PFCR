...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch_A(self, train_data, epoch_idx, client_controls, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            loss.backward()
            self.model_A.pq_code_embedding.weight.grad += client_controls
    ...
    def _train_epoch_B(self, train_data, epoch_idx, client_controls, loss_func=None, show_progress=False):
        self.model_B.train()
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            self._check_nan(loss)
            loss.backward()
            self.model_B.pq_code_embedding.weight.grad += client_controls
            if self.clip_grad_norm:
...
class FedtrainTrainer(Trainer):
    ...
    def fedtrain(self, train_data_A, train_data_B, weight, verbose=True, show_progress=False):
        client_controls_A = torch.zeros_like(self.global_embedding.weight)
        client_controls_B = torch.zeros_like(self.global_embedding.weight)
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            # train
            client_gradients = []
            client_controls_new = []
            training_start_time = time()
            train_loss_A, embedding_grad_A = self._train_epoch_A(train_data_A, epoch_idx, client_controls_A, show_progress=show_progress)
            train_loss_B, embedding_grad_B = self._train_epoch_B(train_data_B, epoch_idx, client_controls_B, show_progress=show_progress)
            ...
            client_controls_A += self.global_embedding.weight - self.model_A.pq_code_embedding.weight
            client_controls_B += self.global_embedding.weight - self.model_B.pq_code_embedding.weight
            client_controls_new.append(client_controls_A)
            client_controls_new.append(client_controls_B)
            ...
            average_control = torch.stack(client_controls_new).mean(0)
            client_controls_A -= average_control
            client_controls_B -= average_control

            self.model_A.pq_code_embedding.weight.data = self.global_embedding.weight.data
            self.model_B.pq_code_embedding.weight.data = self.global_embedding.weight.data
